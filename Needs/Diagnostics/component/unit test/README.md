...
This is a **unit-level** folder — scoped to verifying individual units under `needs/diagnostics/component/unit design/` (e.g. `UNIT_Z_001`) in isolation. It intentionally does not repeat artifact types that already exist once, at the appropriate broader scope, under the root `integration test/` folder:
